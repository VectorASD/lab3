...
def croper(name, img):
  ...
  """ Дизеринг не оправдал себя в многоцветных и при этом больших картинках
  box = img.quantize(colors=256, method=Image.MAXCOVERAGE)
  assert len(box.palette.colors) == 256
  res_img = io.BytesIO()
  box = box.convert("RGB")
  box.save(res_img, format="JPEG", quality=60)
  res2 = res_img.getvalue()
  print("    %s (%s x %s)    %s b. -> %s b.    (%s b.)" % (name, w, h, size, len(res), len(res2)))
  Image.open(io.BytesIO(res2)).show()
  exit()
  """
    ...
    for name in os.listdir():
      ...
      with ZipFile(name, "r") as zip:
        ...
        for event in data:
          ...
          grand_cat = name[5:-4]
          if desc and len(desc) > 135: desc = desc[:135] + "..."
